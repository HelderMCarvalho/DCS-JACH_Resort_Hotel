<?php
    ...
    $sql='SELECT COUNT(*) AS contagemReservas FROM reserva
    WHERE (idTipoQuarto='.$idTipoQuartoIF.')
    AND ((dataCheckIn BETWEEN '.$inicio.' AND '.$fim.')
        OR (dataCheckOut BETWEEN '.$inicio.' AND '.$fim.')
        OR ('.$inicio.' >= dataCheckIn AND '.$fim.' <= dataCheckOut))';
    ...
    var_dump($resultReservas);
    var_dump($resultQuartos);
    if($resultReservas['contagemReservas'] < $resultQuartos['contagemQuartos']){
        ...
        $url = "index.php/?idTipoQuarto=$idTipoQuartoIF";
        header("Location: $url");
    }else{
        echo 'cheio';
